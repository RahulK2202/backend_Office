...
class AdminLoginView(APIView):
    def post(self, request):
        ...
        if user is None:
            return Response({'error': 'No such admin exist'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if not user.is_superuser:
            return Response({'error': 'No admin privileges'}, status=status.HTTP_400_BAD_REQUEST)
...
class EmployeeRegistrationView(APIView):
    def post(self, request, format=None):
        ...
        send_email_to_employee(email,username, employee.id, temporary_password, accessToken)
...
def send_email_to_employee(email, username,user_id, temporary_password,  email_token):
    ...
    verification_link = f'http://localhost:3000/user?token={email_token}&user_id={user_id}'
...
def verify_token(request):
    ...
    try:
        ...
        if(employee.email_token):
            ...
            employee.email_token = ''  # Mark the token as expired (set it to an empty string)
            employee.save()
...
class EmployeeEditView(APIView):
    ...
    def put(self, request, pk):
        try:
            ...
            serializer = EmployeeEditSerializer(employee, data=request.data,partial=True)
...
class AnnouncementEditView(APIView):
    ...
    def delete(self, request, announcement_id):
        try:
            ...
            return JsonResponse({"message": "Announcement not found"}, status=status.HTTP_404_NOT_FOUND)
...
#         user_id = request.data.get('user_id')
#         employee = Employee.objects.get(id=user_id)
#         serializer = EmployeePicSerializer(employee, data=request.data)
#         if serializer.is_valid():
#             serializer.save()
#             return Response(serializer.data)
