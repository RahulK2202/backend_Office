...
class LeaveApplicationView(APIView):
    def post(self, request, format=None):
        ...
        if start_date and start_date < date.today():
            ...
            return Response({'error': 'You cannot apply for leave in the past.'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if employee_id and start_date and end_date:
                ...
                if existing_leave:
                    ...
                    return Response({'error': 'You have already applied for leave during this period.'}, status=status.HTTP_400_BAD_REQUEST)
...
class UserLeaveDataView(APIView):
    def get(self, request, format=None):
        ...
        # Fetch leave data for the specified user
...
class UserLeaveDataView(APIView):
    def get(self, request,employee_id):
        ...
        # Fetch leave data for the specified user
        leave_data = LeaveRequest.objects.filter(employee_id=employee_id,is_approved=True)
