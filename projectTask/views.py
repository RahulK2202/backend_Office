...
class TaskViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        # Validate that the task's start date and end date are within the project's dates
        start_date_str = request_data.get('start_date')
        end_date_str = request_data.get('end_date')
        start_date = datetime.strptime(start_date_str, "%Y-%m-%d").date()
        end_date = datetime.strptime(end_date_str, "%Y-%m-%d").date()
        if start_date < project.start_date or end_date > project.end_date:
            raise ValidationError("Task's start date and end date must be within the project's start date and end date.")
        ...
        # Create the Task instance and save it
        ...
        task.assignedTo.set([assigned_to])  # Assign the Employee instance to the ManyToManyField
    ...
    def get(self, request, *args, **kwargs):
        ...
        # Serialize the list of tasks and return the response
...
def individual_task(request, pk):
    try:
        ...
        serializer = TaskSerializer(tasks, many=True)  # Pass many=True for multiple tasks
        return Response(serializer.data, status=status.HTTP_200_OK)
    except Employee.DoesNotExist:
        return Response({"error": "Employee not found."}, status=status.HTTP_404_NOT_FOUND)
    except Task.DoesNotExist:
        return Response({"error": "No tasks found for the employee."}, status=status.HTTP_404_NOT_FOUND)
...
def update_task_status(request, pk):
    try:
        ...
        return Response({"success": "Task status updated successfully."}, status=status.HTTP_200_OK)
